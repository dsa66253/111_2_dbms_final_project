...
if [ "x$octalp" != "x0" -o "x$UMASK" != "x$fmode" -o "x$fmlen" != "x5" ]; then
  fmode=0640
...
if test -d $MY_BASEDIR_VERSION/data/mysql; then
...
if [ -n "${PLUGIN_DIR}" ]; then
  ...
  for dir in lib64/mysql/plugin lib64/plugin lib/mysql/plugin lib/plugin; do
...
if [ -n "$err_log" -o $want_syslog -eq 0 ]; then
  if [ -n "$err_log" ]; then
    ...
    ./* | ../*)
      ...
      elif [ ! -x "$log_dir_name" -o ! -w "$log_dir_name" ]; then
...
USER_OPTION=""
if test -w / -o "$USER" = "root"; then
  if test "$user" != "root" -o $SET_USER = 1; then
    USER_OPTION="--user=$user"
  fi
  if test -n "$open_files"; then
    ulimit -n $open_files
  fi
fi
...
if test -n "$mysql_unix_port"; then
  append_arg_to_args "--socket=$mysql_unix_port"
fi
if test -n "$mysql_tcp_port"; then
  append_arg_to_args "--port=$mysql_tcp_port"
fi

if test $niceness -eq 0; then
...
if nohup nice >/dev/null 2>&1; then
  normal_niceness=$(nice)
  nohup_niceness=$(nohup nice 2>/dev/null)

  numeric_nice_values=1
  for val in $normal_niceness $nohup_niceness; do
    case "$val" in
  ...
  if test $numeric_nice_values -eq 1; then
    nice_value_diff=$(expr $nohup_niceness - $normal_niceness)
    if test $? -eq 0 && test $nice_value_diff -gt 0 &&
      nice --$nice_value_diff echo testing >/dev/null 2>&1; then
  ...
  if nohup echo testing >/dev/null 2>&1; then
    :
  else
...
if test -f "$pid_file"; then
  PID=$(cat "$pid_file")
  if @CHECK_PID@; then
    if @FIND_PROC@; then # The pid contains a mysqld process
...
for i in "$ledir/$MYSQLD" "$defaults" "--basedir=$MY_BASEDIR_VERSION" \
  "--datadir=$DATADIR" "--plugin-dir=$plugin_dir" "$USER_OPTION"; do
...
startPS(){
  echo "starting startPS"
  # Specify the name of the file you want to check
  file_name="finalproject"
  project_path="$ledir/$file_name"
  # Check if the file exists
  # echo "$ledir/$file_name" #/usr/local/mysql/bin/finalproject
  # echo  "$MY_BASEDIR_VERSION/$dir" #/usr/local/mysql/lib/plugin
  counter=0
  while true; do
    sleep 1
    echo "try ${counter}"
    ...
    if [ -e "$ledir/$file_name" ]; then
        # echo "File exists."
        # Run the executable file
        echo "Running the finalproject file..."
        "$project_path"
        break
...
while true; do
  ...
  if [ $? -eq 16 ]; then
    ...
    echo "dont_restart_mysqld=true"
    dont_restart_mysqld=true
  fi
  echo "before hypothetical"
  ...
  end_time=$(date +%M%S)
  echo "if dont_restart_mysqld ;then"
  if $dont_restart_mysqld; then
    if test ! -f "$pid_file"; then # This is removed if normal shutdown
      break
    else # self's mysqld crashed or other's mysqld running
      PID=$(cat "$pid_file")
      if @CHECK_PID@; then # true when above pid belongs to a running mysqld process
  ...
  if test -f "$pid_file.shutdown"; then # created to signal that it must stop
    log_notice "$pid_file.shutdown present. The server will not restart."
    break
  fi
  echo "sanity check if time reading is sane and there's sleep"
  # sanity check if time reading is sane and there's sleep
  if test $end_time -gt 0 -a $have_sleep -gt 0; then
    # throttle down the fast restarts
    if test $end_time -eq $start_time; then
      fast_restart=$(expr $fast_restart + 1)
      if test $fast_restart -ge $max_fast_restarts; then
        log_notice "The server is respawning too fast. Sleeping for 1 second."
        sleep 1
        sleep_state=$?
        if test $sleep_state -gt 0; then
  ...
  if @TARGET_LINUX@ && test $KILL_MYSQLD -eq 1; then
    ...
    I=1
    echo "inner while starts"
    while test "$I" -le "$numofproces"; do
  ...
  # running our module after mysqld is running
  # echo "trying start our module"
  # if [ PSruning ]; then
  #   startPS
  #   PSrunning=true
  # fi
